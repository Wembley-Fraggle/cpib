...
public class ProcDecl implements IProcDecl {
	...
	public String toString(final String indent) {
		return indent + "<ProcDecl>\n" + ident.toString(indent + '\t')
				+ paramList.toString(indent + '\t') + procDecl1.toString(indent + '\t')
				+ procDecl2.toString(indent + '\t') + procDecl3.toString(indent + '\t')
				+ cpsCmd.toString(indent + '\t') + procDecl4.toString(indent + '\t')
				+ indent + "</ProcDecl>\n";
	}

	@Override
	public IDecl toAbsSyn() {
		return new ch.fhnw.cpib.parsing.abs.impl.ProcDecl(ident, paramList.toAbsSyn(),
				procDecl1.toAbsSyn(), procDecl2.toAbsSyn(), procDecl3.toAbsSyn(),
				cpsCmd.toAbsSyn(), procDecl4.toAbsSyn());
	}
}
