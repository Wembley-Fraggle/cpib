...
public class Parser implements IParser, IConcSyn {
	...
	public ICProgram program() throws GrammarError {
		...
		if ("PROGRAM".equals(name)) {
			...
			return new CProgram(consume(terminal), consume("IDENT"), progParamList(),
					program2(), program3(), consume("DO"), cpsCmd(), program4(),
					consume("ENDPROGRAM"));
		} else {
			throw createError();
		}
	...
	public IProgParamList progParamList() throws GrammarError {
		...
		if ("LPARENT".equals(name)) {
			...
			return new ProgParamList(consume(terminal), progParamList1(),
					consume("RPARENT"));
		} else {
			throw createError();
		}
	...
	public ICpsCmd cpsCmd() throws GrammarError {
		...
		if ("ASSERT".equals(name) || "DEBUGOUT".equals(name)
				...
				|| "WHILE".equals(name) || "IF".equals(name) || "LPARENT".equals(name)
				|| "OLD".equals(name) || "MINUS".equals(name) || "PLUS".equals(name)
	...
	public IFunDecl funDecl() throws GrammarError {
		...
		if ("FUN".equals(name)) {
			...
			return new FunDecl(consume(terminal), consume("IDENT"), paramList(),
					consume("RETURNS"), stoDecl(), funDecl1(), funDecl2(), funDecl3(),
					consume("DO"), cpsCmd(), funDecl4(), consume("ENDFUN"));
		} else {
			throw createError();
		}
	}
	...
	public IFunDecl1 funDecl1() throws GrammarError {
		...
		} else if ("DO".equals(name) || "PRE".equals(name) || "LOCAL".equals(name)) {
	...
	public IProcDecl procDecl() throws GrammarError {
		...
		if ("PROC".equals(name)) {
			...
			return new ProcDecl(consume(terminal), consume("IDENT"), paramList(),
					procDecl1(), procDecl2(), procDecl3(), consume("DO"), cpsCmd(),
					procDecl4(), consume("ENDPROC"));
		} else {
			throw createError();
		}
	}
	...
	public ITypedIdent typedIdent() throws GrammarError {
		...
		if ("IDENT".equals(name)) {
			...
			return new TypedIdent(consume(terminal), consume("COLON"), atomtype());
		} else {
			throw createError();
		}
	}
	...
	public IParamList paramList() throws GrammarError {
		...
		if ("LPARENT".equals(name)) {
			...
			return new ParamList(consume(terminal), paramList1(), consume("RPARENT"));
		} else {
			throw createError();
		}
	}
	...
	public IParamList1 paramList1() throws GrammarError {
		String name = terminal.getName();
		if ("IDENT".equals(name) || "VAR".equals(name) || "CONST".equals(name)
				|| "REF".equals(name) || "COPY".equals(name) || "OUT".equals(name)
				|| "INOUT".equals(name) || "IN".equals(name)) {
	...
	public IParam1 param1() throws GrammarError {
		...
		if ("OUT".equals(name) || "INOUT".equals(name) || "IN".equals(name)) {
			...
			return new Param1(flowmode());
				...
				|| "CONST".equals(name) || "REF".equals(name) || "COPY".equals(name)) {
			LOG.debug("param1 ::= <e>");
	...
	public IConcExpr expr() throws GrammarError {
		String name = terminal.getName();
		if ("LPARENT".equals(name) || "OLD".equals(name) || "MINUS".equals(name)
				|| "PLUS".equals(name) || "NOT".equals(name) || "IDENT".equals(name)
				|| "INTVAL32".equals(name) || "TRUE".equals(name)
				|| "FALSE".equals(name)) {
	...
	public ICExprList exprList() throws GrammarError {
		...
		if ("LPARENT".equals(name)) {
			...
			return new ExprList(consume(terminal), exprList1(), consume("RPARENT"));
		} else {
			throw createError();
		}
	}
	...
	public ICCmd cmd() throws GrammarError {
		...
		if ("SKIP".equals(name)) {
			...
			return new CmdSkip(consume(terminal));
				...
				|| "MINUS".equals(name) || "PLUS".equals(name) || "NOT".equals(name)
				|| "IDENT".equals(name) || "INTVAL32".equals(name)
	...
	public IExpr1 expr1() throws GrammarError {
		...
		if ("COR".equals(name) || "CAND".equals(name) || "OR".equals(name)
			...
			return new Expr1(expr2(), expr1());
				...
				|| "DO".equals(name) || "INV".equals(name) || "THEN".equals(name)
				|| "ENDWHILE".equals(name) || "ENDIF".equals(name)
				|| "ELSE".equals(name) || "ENDPROC".equals(name)
				|| "ENDFUN".equals(name) || "ENDPROGRAM".equals(name)
				|| "POST".equals(name) || "SEMI".equals(name) || "BECOMES".equals(name)) {
			LOG.debug("expr1 ::= <e>");
			return new Expr1Eps();
		} else {
			throw createError();
	...
	public IExpr2 expr2() throws GrammarError {
		...
		if ("COR".equals(name) || "CAND".equals(name) || "OR".equals(name)
			...
			return new Expr2(boolopr(), term1());
		} else {
			throw createError();
		}
	}
	...
	public IBoolopr boolopr() throws GrammarError {
		...
		if ("AND".equals(name)) {
			...
			return new BooloprAnd(consume(terminal));
		} else if ("OR".equals(name)) {
			LOG.debug("boolopr ::= OR");
			return new BooloprOr(consume(terminal));
		...
		if ("COR".equals(name)) {
			...
			return new BooloprCor(consume(terminal));
		} else {
			throw createError();
		}
	}
	...
	public ITerm12 term12() throws GrammarError {
		...
		if ("GE".equals(name) || "LE".equals(name) || "GT".equals(name)
			...
			return new Term12(relopr(), term2());
		} else {
			throw createError();
		}
	}
	...
	public ITerm11 term11() throws GrammarError {
		...
		if ("GE".equals(name) || "LE".equals(name) || "GT".equals(name)
			...
			return new Term11(term12());
				...
				|| "ENDFUN".equals(name) || "ENDPROGRAM".equals(name)
				|| "POST".equals(name) || "SEMI".equals(name) || "BECOMES".equals(name)
				|| "COR".equals(name) || "CAND".equals(name) || "OR".equals(name)
				|| "AND".equals(name)) {
			LOG.debug("term11 ::= <e>");
	...
	public IRelopr relopr() throws GrammarError {
		...
		if ("EQ".equals(name)) {
			LOG.debug("relopr ::= EQ");
			return new Relopr(consume(terminal));
		} else if ("NE".equals(name)) {
			LOG.debug("relopr ::= NE");
			return new ReloprNe(consume(terminal));
		} else if ("LT".equals(name)) {
			LOG.debug("relopr ::= LT");
			return new ReloprLt(consume(terminal));
		} else if ("GT".equals(name)) {
			LOG.debug("relopr ::= GT");
			return new ReloprGt(consume(terminal));
		} else if ("LE".equals(name)) {
			LOG.debug("relopr ::= LE");
			return new ReloprLe(consume(terminal));
		} else if ("GE".equals(name)) {
			LOG.debug("relopr ::= GE");
			return new ReloprGe(consume(terminal));
		}
	...
	public ITerm3 term3() throws GrammarError {
		...
		if ("LPARENT".equals(name) || "OLD".equals(name) || "MINUS".equals(name)
			...
			return new Term3(factor(), term31());
		} else {
			throw createError();
		}
	}
	...
	public ITerm21 term21() throws GrammarError {
		...
		if ("MINUS".equals(name) || "PLUS".equals(name)) {
			...
			return new Term21(term211(), term21());
		} else if ("COMMA".equals(name) || "RPARENT".equals(name)
				|| "DO".equals(name) || "INV".equals(name) || "THEN".equals(name)
				|| "ENDWHILE".equals(name) || "ENDIF".equals(name)
				|| "ELSE".equals(name) || "ENDPROC".equals(name)
				...
				|| "POST".equals(name) || "SEMI".equals(name) || "BECOMES".equals(name)
				|| "COR".equals(name) || "CAND".equals(name) || "OR".equals(name)
				|| "AND".equals(name) || "GE".equals(name) || "LE".equals(name)
	...
	public ITerm31 term31() throws GrammarError {
		...
		if ("MOD".equals(name) || "DIV".equals(name) || "TIMES".equals(name)) {
			...
			return new Term31(term311(), term31());
		} else if ("COMMA".equals(name) || "RPARENT".equals(name)
				|| "DO".equals(name) || "INV".equals(name) || "THEN".equals(name)
				|| "ENDWHILE".equals(name) || "ENDIF".equals(name)
				|| "ELSE".equals(name) || "ENDPROC".equals(name)
				...
				|| "AND".equals(name) || "GE".equals(name) || "LE".equals(name)
				|| "GT".equals(name) || "LT".equals(name) || "NE".equals(name)
				|| "EQ".equals(name) || "MINUS".equals(name) || "PLUS".equals(name)) {
			LOG.debug("term31 ::= <e>");
	...
	public IMultopr multopr() throws GrammarError {
		...
		if ("TIMES".equals(name)) {
			...
			return new MultoprTimes(consume(terminal));
		} else if ("DIV".equals(name)) {
			LOG.debug("multopr ::= DIV");
			return new MultoprDiv(consume(terminal));
		} else if ("MOD".equals(name)) {
			LOG.debug("multopr ::= MOD");
			return new MultoprMod(consume(terminal));
		} else {
			throw createError();
		}
	}
	...
	public IFactor21 factor21() throws GrammarError {
		...
		if ("LPARENT".equals(name) || "INIT".equals(name)) {
			...
			return new Factor21(factor211());
				...
				|| "GT".equals(name) || "LT".equals(name) || "NE".equals(name)
				|| "EQ".equals(name) || "MINUS".equals(name) || "PLUS".equals(name)
				|| "MOD".equals(name) || "DIV".equals(name) || "TIMES".equals(name)) {
	...
	public IFactor3 factor3() throws GrammarError {
		...
		if ("MINUS".equals(name) || "PLUS".equals(name) || "NOT".equals(name)) {
			...
			return new Factor3(monadicOpr(), factor());
		} else {
			throw createError();
		}
	}
	...
	public IFactor4 factor4() throws GrammarError {
		...
		if ("OLD".equals(name)) {
			...
			return new Factor4(consume(terminal), consume("LPARENT"), expr(),
					consume("RPARENT"));
		} else {
			throw createError();
		}
	...
	public IFactor5 factor5() throws GrammarError {
		...
		if ("LPARENT".equals(name)) {
			...
			return new Factor5(consume(terminal), expr(), consume("RPARENT"));
		} else {
			throw createError();
		}
	}
	...
	public IMonadicOpr monadicOpr() throws GrammarError {
		...
		if ("NOT".equals(name)) {
			...
			return new MonadicOprNot(consume(terminal));
		} else if ("MINUS".equals(name) || "PLUS".equals(name)) {
			LOG.debug("monadicOpr ::= addopr");
			return new MonadicOprAdd(addopr());
	...
	public IExprList1 exprList1() throws GrammarError {
		...
		if ("LPARENT".equals(name) || "OLD".equals(name) || "MINUS".equals(name)
			...
			return new ExprList1(exprList2());
		} else if ("RPARENT".equals(name)) {
	...
	public IExprList2 exprList2() throws GrammarError {
		...
		if ("LPARENT".equals(name) || "OLD".equals(name) || "MINUS".equals(name)
			...
			return new ExprList2(expr(), exprList3());
		} else {
			throw createError();
		}
	}
	...
	public IExprList3 exprList3() throws GrammarError {
		...
		if ("COMMA".equals(name)) {
			...
			return new ExprList3(exprList4(), exprList3());
		} else if ("RPARENT".equals(name)) {
	...
	public IExprList4 exprList4() throws GrammarError {
		...
		if ("COMMA".equals(name)) {
			...
			return new ExprList4(consume(terminal), expr());
		} else {
			throw createError();
		}
	}
	...
	public ICmd1 cmd1() throws GrammarError {
		...
		if ("LPARENT".equals(name) || "OLD".equals(name) || "MINUS".equals(name)
			...
			return new Cmd1(expr(), consume("BECOMES"), expr());
		} else {
			throw createError();
		}
	}
	...
	public ICmd2 cmd2() throws GrammarError {
		...
		if ("IF".equals(name)) {
			...
			return new Cmd2(consume(terminal), expr(), consume("THEN"), cpsCmd(),
					consume("ELSE"), cpsCmd(), consume("ENDIF"));
		} else {
			throw createError();
		}
	}
	...
	public ICmd3 cmd3() throws GrammarError {
		...
		if ("WHILE".equals(name)) {
			...
			return new Cmd3(consume(terminal), expr(), cmd31(), consume("DO"),
					cpsCmd(), consume("ENDWHILE"));
		} else {
			throw createError();
		}
	}
	...
	public ICmd4 cmd4() throws GrammarError {
		...
		if ("CALL".equals(name)) {
			...
			return new Cmd4(consume(terminal), consume("IDENT"), exprList());
	...
	public ICmd7 cmd7() throws GrammarError {
		...
		if ("ASSERT".equals(name)) {
			...
			return new Cmd7(consume(terminal), consume("IDENT"), consume("COLON"),
					expr());
		} else {
			throw createError();
		}
	...
	public IPostcondition postcondition() throws GrammarError {
		...
		if ("POST".equals(name)) {
			...
			return new Postcondition(consume(terminal), consume("IDENT"),
					consume("COLON"), expr());
		} else {
			throw createError();
		}
	...
	public IInvariant invariant() throws GrammarError {
		...
		if ("INV".equals(name)) {
			...
			return new Invariant(consume(terminal), consume("IDENT"),
					consume("COLON"), (IConcExpr) expr());
		} else {
			throw createError();
		}
	}
